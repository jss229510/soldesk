...
OUTPUT_PATH = BASE_DIR / "data" / "psu_playwright.csv"
PARSER_DIR = BASE_DIR.parent
if str(PARSER_DIR) not in sys.path:
    sys.path.insert(0, str(PARSER_DIR))

import danawa_psu_crawler as psu_parser


def pcode_from_url(url):
    match = re.search(r"pcode=(\d+)", url or "")
    return match.group(1) if match else None


def makers_by_pcode(html):
    """Next.js 원본 payload의 makerName을 다나와 상품번호별로 보관한다."""
    products, _ = psu_parser.parse_next_payload(html)
    makers = {}
    for product in products or []:
        maker = (product.get("makerName") or "").strip() or None
        bundles = product.get("bundleProducts") or [{"productCode": product.get("id")}]
        for bundle in bundles:
            pcode = bundle.get("productCode") or product.get("id")
            if pcode and maker:
                makers[str(pcode)] = maker
    return makers
...
def read_products(page):
    ...
    for product in soup.select('div[data-testid="ProductListItem"]'):
        ...
        results.append({
            "제조사": makers.get(pcode_from_url(product_url)),
