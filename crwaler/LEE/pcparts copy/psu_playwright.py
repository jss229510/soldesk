...
def psu_run():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            size_label = page.locator("label:visible").filter(has=page.locator('span[title="ATX 파워"]'))
            size_id = size_label.get_attribute("for")
            size_checkbox = page.locator(f'[id={size_id}]')

            size_checkbox.check()
            page.wait_for_timeout(2000)

            for maker in TARGET_MANUFACTURERS['PSU']:
                if maker == "마이크로닉스":
                    maker_label = page.locator("label:visible").filter(has=page.locator('img[alt="마이크로닉스"]'))
                    checkbox_id = maker_label.get_attribute("for")
                    maker_checkbox = page.locator(f'[id={checkbox_id}]')
                else:
